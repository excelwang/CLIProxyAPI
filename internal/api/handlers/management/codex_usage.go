...
func buildCodexUsageRecoveryWindow(now time.Time, contributions []codexRecoveryContribution) *codexUsageRecoveryWindow {
	...
	for _, item := range contributions {
		...
		usedPercent := clampRecoveryPercent(float64(item.window.UsedPercent))
		blockedUnits := weight * usedPercent / 100
		if blockedUnits <= 0 {
			continue
		}
		lockedUnits += blockedUnits
		resetAt, ok := codexWindowResetAt(item.window, now)
		if !ok || resetAt <= 0 {
			continue
		}
		releaseByReset[resetAt] += blockedUnits
	}
	if totalUnits <= 0 {
		return nil
	}
	availableNow := totalUnits - lockedUnits
	if availableNow < 0 {
		availableNow = 0
	}
	out := &codexUsageRecoveryWindow{
		TotalUnits:          roundFloat(totalUnits, 4),
		LockedUnits:         roundFloat(lockedUnits, 4),
		AvailableUnitsNow:   roundFloat(availableNow, 4),
		AvailablePercentNow: clampRecoveryPercent(availableNow / totalUnits * 100),
	}
	...
	cumulativeRelease := 0.0
	significantTargetUnits := math.Max(1.0, totalUnits*0.5)
	for _, resetAt := range resetAts {
		releaseUnits := releaseByReset[resetAt]
		if releaseUnits <= 0 {
			continue
		}
		cumulativeRelease += releaseUnits
		availableUnits := availableNow + cumulativeRelease
		if availableUnits > totalUnits {
			availableUnits = totalUnits
		}
		waitSeconds := maxInt(0, int(resetAt-now.Unix()))
		event := codexUsageRecoveryEvent{
			ResetAt:                  resetAt,
			WaitSeconds:              waitSeconds,
			ReleaseUnits:             roundFloat(releaseUnits, 4),
			CumulativeReleaseUnits:   roundFloat(cumulativeRelease, 4),
			CumulativeReleasePercent: clampRecoveryPercent(cumulativeRelease / totalUnits * 100),
			AvailableUnits:           roundFloat(availableUnits, 4),
			AvailablePercent:         clampRecoveryPercent(availableUnits / totalUnits * 100),
		}
		out.Events = append(out.Events, event)

		if out.NextResetAt == 0 {
			out.NextWaitSeconds = waitSeconds
			out.NextResetAt = resetAt
			out.NextReleaseUnits = roundFloat(releaseUnits, 4)
			out.NextAvailableUnits = roundFloat(availableUnits, 4)
			out.NextAvailablePercent = event.AvailablePercent
		}
		if out.SignificantResetAt == 0 && availableUnits >= significantTargetUnits {
			out.SignificantWaitSeconds = waitSeconds
			out.SignificantResetAt = resetAt
			out.SignificantTargetUnits = roundFloat(significantTargetUnits, 4)
			out.SignificantReleaseUnits = roundFloat(cumulativeRelease, 4)
			out.SignificantAvailableUnits = roundFloat(availableUnits, 4)
			out.SignificantAvailablePercent = event.AvailablePercent
		}
		out.FullWaitSeconds = waitSeconds
		out.FullResetAt = resetAt
		out.FullReleaseUnits = roundFloat(cumulativeRelease, 4)
		out.FullAvailableUnits = roundFloat(availableUnits, 4)
		out.FullAvailablePercent = event.AvailablePercent
	}
	return out
}
...
func buildCodexUsageCombinedRecovery(now time.Time, current map[string]codexAuthUsageStatus, freePlanWeight, proPlanWeight float64) *codexUsageCombinedRecovery {
	...
	totalUnits := 0.0
	availableUnitsNow := 0.0
	releaseByReset := make(map[int64]float64)
	fiveHourBlockedUnitsNow := 0.0
	fiveHourReleaseByReset := make(map[int64]float64)
	for _, status := range current {
		planType := strings.TrimSpace(status.PlanType)
		if planType == "" && status.Usage != nil {
			planType = strings.TrimSpace(status.Usage.PlanType)
		...
		normalized := codexEffectiveMainRateLimit(now, status)
		if normalized == nil {
			availableUnitsNow += weight
			continue
		}

		weekWindow := normalized.SecondaryWindow
		if weekWindow == nil && normalized.PrimaryWindow != nil && isLikelyCodexWeeklyWindow(normalized.PrimaryWindow.LimitWindowSeconds) {
			weekWindow = normalized.PrimaryWindow
		}
		if weekWindow == nil && strings.EqualFold(planType, "free") {
			weekWindow = normalized.PrimaryWindow
		}
		weekAvailable, weekLocked := codexSplitRecoveryUnits(weight, weekWindow)
		availableUnitsNow += weekAvailable
		if weekLocked > 0 {
			if resetAt, ok := codexWindowResetAt(weekWindow, now); ok && resetAt > 0 {
				releaseByReset[resetAt] += weekLocked
			}
		}

		fiveWindow := normalized.PrimaryWindow
		if fiveWindow == nil && strings.EqualFold(planType, "free") {
			fiveWindow = weekWindow
		}
		if codexRecoveryWindowBlocked(fiveWindow) {
			_, fiveLocked := codexSplitRecoveryUnits(weight, fiveWindow)
			fiveHourBlockedUnitsNow += fiveLocked
			if fiveLocked > 0 {
				if resetAt, ok := codexWindowResetAt(fiveWindow, now); ok && resetAt > 0 {
					fiveHourReleaseByReset[resetAt] += fiveLocked
				}
			}
		}
	}
	...
	if availableUnitsNow < 0 {
		availableUnitsNow = 0
	}
	if availableUnitsNow > totalUnits {
		availableUnitsNow = totalUnits
	}

	out := &codexUsageCombinedRecovery{
		TotalUnits:                roundFloat(totalUnits, 4),
		AvailableUnitsNow:         roundFloat(availableUnitsNow, 4),
		AvailablePercentNow:       clampRecoveryPercent(availableUnitsNow / totalUnits * 100),
		FiveHourBlockedUnitsNow:   roundFloat(fiveHourBlockedUnitsNow, 4),
		FiveHourBlockedPercentNow: clampRecoveryPercent(fiveHourBlockedUnitsNow / totalUnits * 100),
	}
	...
	sort.Slice(resetAts, func(i, j int) bool { return resetAts[i] < resetAts[j] })
	cumulativeRelease := 0.0
	significantDelta := math.Max(1.0, totalUnits*0.5)
	for _, resetAt := range resetAts {
		releaseUnits := releaseByReset[resetAt]
		if releaseUnits <= 0 {
			continue
		...
		cumulativeRelease += releaseUnits
		availableUnits := availableUnitsNow + cumulativeRelease
		if availableUnits > totalUnits {
			availableUnits = totalUnits
		}
		waitSeconds := maxInt(0, int(resetAt-now.Unix()))
		...
		if out.SignificantResetAt == 0 && cumulativeRelease >= significantDelta {
			out.SignificantDeltaUnits = roundFloat(significantDelta, 4)
			out.SignificantWaitSeconds = waitSeconds
			out.SignificantResetAt = resetAt
			out.SignificantAvailableUnits = roundFloat(availableUnits, 4)
			out.SignificantAvailablePercent = event.AvailablePercent
	...
	fiveHourResetAts := make([]int64, 0, len(fiveHourReleaseByReset))
	for resetAt := range fiveHourReleaseByReset {
		fiveHourResetAts = append(fiveHourResetAts, resetAt)
	}
	sort.Slice(fiveHourResetAts, func(i, j int) bool { return fiveHourResetAts[i] < fiveHourResetAts[j] })
	if len(fiveHourResetAts) > 0 {
		out.FiveHourNextResetAt = fiveHourResetAts[0]
		out.FiveHourNextWaitSeconds = maxInt(0, int(fiveHourResetAts[0]-now.Unix()))
	}
	return out
}
...
func codexSplitRecoveryUnits(weight float64, window *codexUsageWindow) (available float64, locked float64) {
	...
	locked = weight * usedPercent / 100
	available = weight - locked
	if available < 0 {
		available = 0
	}
...
func aggregateCodexUsage(authStatuses map[string]codexAuthUsageStatus, freePlanWeight, proPlanWeight float64) (codexUsagePayload, codexUsageTotalSummary, int) {
	...
	for _, status := range authStatuses {
		...
		for _, item := range usage.AdditionalRateLimits {
			key := strings.ToLower(strings.TrimSpace(item.LimitName)) + "\x00" + strings.ToLower(strings.TrimSpace(item.MeteredFeature))
			...
			acc.rate.addDenominator(weight)
			acc.rate.add(normalizeCodexRateLimitWindows(item.RateLimit), weight)
		}

		if compat.UserID == "" {
			compat.UserID = strings.TrimSpace(usage.UserID)
		}
		if compat.AccountID == "" {
			compat.AccountID = strings.TrimSpace(usage.AccountID)
		}
		if compat.Email == "" {
			compat.Email = strings.TrimSpace(usage.Email)
		}
	}

	compat.PlanType = dominantPlanType(planCounts)
	compat.TotalUsageMultiplier = roundFloat(ensureCodexTotalUsageMultiplierValue(compat.TotalUsageMultiplier, compat.PlanType, freePlanWeight, proPlanWeight), 4)
	compat.RateLimit = normalizeCodexMainRateLimitWindows(mainRate.averageRateLimit())
	compat.CodeReviewRateLimit = normalizeCodexRateLimitWindows(codeReviewRate.averageRateLimit())
	if hasCreditsPayload {
		...
		if balanceCount > 0 {
			balance := strconv.FormatFloat(roundFloat(balanceSum/float64(balanceCount), 4), 'f', -1, 64)
	...
	if len(additional) > 0 {
		...
		for _, key := range keys {
			...
			compat.AdditionalRateLimits = append(compat.AdditionalRateLimits, codexUsageAdditionalRateLimit{
				...
				RateLimit:      normalizeCodexRateLimitWindows(acc.rate.averageRateLimit()),
			})
		}
	}

	totals.TotalUsageMultiplier = compat.TotalUsageMultiplier
...
func dominantPlanType(planCounts map[string]int) string {
	...
	items := make([]planCount, 0, len(planCounts))
	for plan, count := range planCounts {
		items = append(items, planCount{plan: plan, count: count})
	}
	sort.Slice(items, func(i, j int) bool {
		if items[i].count == items[j].count {
			return items[i].plan < items[j].plan
		}
...
func roundPercentToInt(progress float64) int {
	return int(math.Round(clampRecoveryPercent(progress)))
}
...
func normalizeCodexRateLimitWindows(rate *codexUsageRateLimit) *codexUsageRateLimit {
	...
	if out.PrimaryWindow != nil {
		if out.PrimaryWindow.LimitWindowSeconds <= 0 && out.SecondaryWindow == nil {
			out.PrimaryWindow.LimitWindowSeconds = codexFiveHourWindowSecs
		}
	}
	if out.SecondaryWindow != nil && out.SecondaryWindow.LimitWindowSeconds <= 0 {
		out.SecondaryWindow.LimitWindowSeconds = codexWeeklyWindowSecs
	}
	return out
}
...
func normalizeCodexMainRateLimitWindows(rate *codexUsageRateLimit) *codexUsageRateLimit {
	...
	out := normalizeCodexRateLimitWindows(rate)
	if out == nil {
		return nil
	}
	if out.PrimaryWindow != nil && isLikelyCodexWeeklyWindow(out.PrimaryWindow.LimitWindowSeconds) && out.SecondaryWindow == nil {
		// Some free-plan payloads expose the weekly window as the only/main window.
		out.SecondaryWindow = cloneCodexUsageWindow(out.PrimaryWindow)
	}
	if out.PrimaryWindow == nil && out.SecondaryWindow != nil {
		out.PrimaryWindow = cloneCodexUsageWindow(out.SecondaryWindow)
		if out.PrimaryWindow != nil && out.PrimaryWindow.LimitWindowSeconds <= 0 {
			out.PrimaryWindow.LimitWindowSeconds = codexFiveHourWindowSecs
		}
	}
	return out
...
func isLikelyCodexFiveHourWindow(seconds int) bool {
	return seconds > 0 && seconds <= codexFiveHourWindowSecs+(30*60)
}

func isLikelyCodexWeeklyWindow(seconds int) bool {
	return seconds >= codexWeeklyWindowSecs-(6*60*60)
}
...
func ensureCodexTotalUsageMultiplier(payload *codexUsagePayload, freePlanWeight, proPlanWeight float64) {
	...
	payload.TotalUsageMultiplier = roundFloat(ensureCodexTotalUsageMultiplierValue(payload.TotalUsageMultiplier, payload.PlanType, freePlanWeight, proPlanWeight), 4)
}

func (h *Handler) codexFreePlanWeight() float64 {
	if overrides := h.codexUsageConfigOverrides(); overrides.CodexFreePlanWeight > 0 {
...
func (h *Handler) codexProPlanWeight() float64 {
	if overrides := h.codexUsageConfigOverrides(); overrides.CodexProPlanWeight > 0 {
...
func (h *Handler) codexOAuthAvailableTotalForAPIKey(apiKey string) (units float64, configured bool, matched bool) {
	...
	if len(overrides.CodexOAuthAvailableTotals) == 0 {
		return 0, false, false
	}
	if h == nil || h.cfg == nil {
		return 0, true, false
	...
	for i := range h.cfg.CodexKey {
		...
		if i >= 0 && i < len(overrides.CodexOAuthAvailableTotals) {
			return overrides.CodexOAuthAvailableTotals[i], true, true
		}
		return 0, true, true
	}
	return 0, true, false
...
func (h *Handler) EvaluateCodexOAuthQuota(ctx context.Context, apiKey string) (exceeded bool, used float64, limit float64, checked bool) {
	limit, configured, matched := h.codexOAuthAvailableTotalForAPIKey(apiKey)
	if !configured || !matched || limit <= 0 {
		return false, 0, limit, configured && matched
	}
	...
	for _, status := range snapshot {
		...
		if strings.TrimSpace(status.AuthID) == "" {
			continue
		}
		auth, ok := h.authManager.GetByID(strings.TrimSpace(status.AuthID))
		if !ok || auth == nil || auth.Attributes == nil {
			continue
		}
		if strings.TrimSpace(auth.Attributes["api_key"]) != strings.TrimSpace(apiKey) {
			continue
		}
		rate := codexEffectiveMainRateLimit(time.Now().UTC(), status)
		if rate == nil || rate.SecondaryWindow == nil {
			continue
		}
		used = limit * clampRecoveryPercent(float64(rate.SecondaryWindow.UsedPercent)) / 100
		return used >= limit, roundFloat(used, 4), limit, true
	}
...
func inferCodexPlanType(auth *coreauth.Auth, status codexAuthUsageStatus) string {
	if status.Usage != nil {
		if planType := strings.TrimSpace(status.Usage.PlanType); planType != "" {
			return planType
		}
	}
	if status.PlanType != "" {
		return strings.TrimSpace(status.PlanType)
	}
	if auth == nil {
		return "guest"
	}
	...
	if auth.Metadata != nil {
		for _, key := range []string{"plan_type", "planType"} {
			if value, ok := auth.Metadata[key]; ok {
				switch v := value.(type) {
				case string:
					if planType := strings.TrimSpace(v); planType != "" {
						return planType
					}
				}
			}
